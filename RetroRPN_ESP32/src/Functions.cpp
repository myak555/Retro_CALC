...
void Functions::init( void *components[]){
  ...
  _setFunction( _OPR_AZIMUTH3_KW, (void *)_function_Solver_AZIMUTH3_, 3);
...
Function *Functions::findByName(byte *str){
  Keyword *kwd = _kwds->findByName( str);
  return findByKeyword(str, kwd);
}

Function *Functions::findByKeyword(byte *str, Keyword *kwd){
  Function *ptr;
  if( kwd == NULL){
...
void Functions::_addFunction( int16_t kwid, byte nArgs){
  ...
  tmp->nArgs = nArgs;
  #ifdef __DEBUG
  Keyword *kwd = _kwds->findById( kwid);
